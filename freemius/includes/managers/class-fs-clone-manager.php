...
class FS_Clone_Manager {
	...
	function _init() {
		if ( is_admin() ) {
			if ( Freemius::is_admin_post() ) {
				add_action( 'admin_post_fs_clone_resolution', [ $this, '_handle_clone_resolution' ] );
			}

			if ( Freemius::is_ajax() ) {
				Freemius::add_ajax_action_static( 'handle_clone_resolution', [ $this, '_clone_resolution_action_ajax_handler' ] );
			} elseif (
				...
				add_action( 'admin_footer', [ $this, '_add_clone_resolution_javascript' ] );
			}
		}
	}
	...
	private function try_resolve_clone_automatically_by_instance(
		...
		if ( is_object( $associated_install ) ) {
			...
			$instance->sync_install( [ 'is_new_site' => true ], true );
	...
	function store_blog_install_info( $blog_id, $site = null ) {
		...
		$new_blog_install_map[ $blog_id ] = [ 'install_id' => $install_id ];
	...
	function _add_clone_resolution_javascript() {
		$vars = [ 'ajax_action' => Freemius::get_ajax_action_static( 'handle_clone_resolution' ) ];
	...
	function _clone_resolution_action_ajax_handler() {
		...
		$result = $this->resolve_cloned_sites( $clone_action, [], $blog_id );
	 ...
	 * @return array
	 */
	private function resolve_cloned_sites( $clone_action, $fs_instances = [], $blog_id = 0 ) {
		$this->_logger->entrance();

		$result = [];

		$instances_with_clone       = [];
		$instances_with_clone_count = 0;
		$install_by_instance_id     = [];
