...
class FS_Checkout_Manager {
	 ...
	 */
	private $_allowed_custom_params = [
		// currency
		'currency'                      => true,
	...
	public function get_query_params( Freemius $fs, $plugin_id, $plan_id, $licenses ) {
		...
		if ( $fs->is_registered() ) {
			...
			$context_params = array_merge( $context_params, [
		...
		return array_merge( $context_params, $filtered_params, $_GET, [
	...
	public function get_install_url( Freemius $fs, $plugin_id ) {
		return fs_nonce_url( $fs->_get_admin_page_url( 'account', [
