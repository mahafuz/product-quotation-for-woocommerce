...
class FS_DebugManager {
	...
	static function _add_debug_section() {
		...
		if ( ! empty( $hook ) ) {
			add_action( "load-$hook", [ self::class, '_debug_page_actions' ] );
		}
	}
	...
	public static function load_required_static() {
		if ( ! WP_FS__DEMO_MODE ) {
			add_action( ( fs_is_network_admin() ? 'network_' : '' ) . 'admin_menu', [
				self::class,
				'_add_debug_section',
			] );
		}

		add_action( 'wp_ajax_fs_toggle_debug_mode', [ self::class, '_toggle_debug_mode' ] );

		Freemius::add_ajax_action_static( 'get_debug_log', [ self::class, '_get_debug_log' ] );
		Freemius::add_ajax_action_static( 'get_db_option', [ self::class, '_get_db_option' ] );
		Freemius::add_ajax_action_static( 'set_db_option', [ self::class, '_set_db_option' ] );
	}
	...
	public static function register_hooks() {
		add_action( 'fs_debug_turn_off_logging_hook', [ self::class, '_turn_off_debug_mode' ] );
	}
