...
class FS_Admin_Notice_Manager {
	...
	protected function __construct(
		...
		if ( ( $this->_is_network_notices && $is_network_admin ) ||
			...
			if ( 0 < count( $this->_sticky_storage ) ) {
				...
				add_action( "wp_ajax_fs_dismiss_notice_action_{$ajax_action_suffix}", [
					&$this,
	...
	function _admin_notices_hook() {
		...
		foreach ( $this->_notices as $id => $msg ) {
			 ...
			 * @since 2.2.0
			 */
			$show_notice = call_user_func_array( 'fs_apply_filter', [
	 ...
	 * @param array       $data
	 */
	function add_sticky( $message, $id, $title = '', $type = 'success', $wp_user_id = null, $plugin_title = null, $is_network_and_blog_admins = false, $is_dimissible = true, $data = [] ) {
