...
class Helper {
	 ...
	 */
	public static function generatePrivacyPolicy( $text ) {
	...
	public static function validate( $fields ) {
		...
		foreach ( $requiredFields as $required ) {
			if ( empty( $fields[ $required ] ) ) {
				$errors->add( 'field', sprintf( '%s %s', ucfirst( $required ), __( 'is required.', 'quotify' ) ) );
			}
		}
	 ...
	 * @since 2.0.1
	 */
	public static function getCart( $field = 'id' ) {
	 ...
	 */
	public static function escape_html_form( $html ) {
		$allowed_tags = [
			'form'     => [
				'action'  => true,
				'method'  => true,
	 ...
	 */
	public static function get_post_meta_by_id( $id ) {
		global $wpdb;
	 ...
	 */
	public static function pageLookUp( $screen = null ) {
		$page = false;
		...
		if ( ! $page ) {
			$page = ! empty( $_GET['page'] ) ? sanitize_key( wp_unslash( $_GET['page'] ) ) : false; //phpcs:ignore
		}
		...
		$slugs = \Quotify\Admin::REGISTERED_SLUGS;
	 ...
	 *
	 * @since 2.5.0
	 * @return html The preloader html.
	 */
