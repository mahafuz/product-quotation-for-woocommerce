...
function Index({ id }) {
	...
	useEffect(() => {
		if (!quotation?.data && id) {
			setLoading(true);
			dispatch(getQuote(id)).then(() => setLoading(false));
		} else {
			setLoading(false);
		}
	}, [id, quotation?.data]);
		...
		<>
			...
			<div className="quotify-content-wrap quote-container">
				<div className="quotify-card">
					<h2 className={`quotify-card-title`}>{quotation?.title || 'Quote'}</h2>
				...
				<div className="quotify-card">
					...
					<p><strong>Name:</strong> {meta.pqfw_customer_name}</p>
					<p><strong>Email:</strong> {meta.pqfw_customer_email}</p>
					{meta.pqfw_customer_phone && (
						<p><strong>Phone:</strong> {meta.pqfw_customer_phone}</p>
					)}
					<p><strong>Subject:</strong> {meta.pqfw_customer_subject}</p>
					{meta.pqfw_customer_comments && (
						<p><strong>Comments:</strong> {meta.pqfw_customer_comments}</p>
					)}
				</div>
				...
				<div className="quotify-card">
					...
					{Array.isArray(meta.pqfw_products_info) &&
						meta.pqfw_products_info.map(({ name, price, img, link, quantity, message}, index) => (
							<div className="product-item" key={index}>
								...
								<div className="product-info">
									...
									<p><strong>Price:</strong> <span dangerouslySetInnerHTML={{ __html:price }} /></p>
